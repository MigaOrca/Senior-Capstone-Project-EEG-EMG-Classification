...
For all jobs submitted via the terminal or the shell via OpenOnDemand (OOD), make sure to run the following lines:
```
...
To run on Compute1, the command in the terminal should look like the following:
```
...
```
Memory usage (rusage) can be altered as needed, though I found that 16 GB is about the minimum memory it needs for 18 experiments.  
...
Note: Due to time constraints, no full model training was performed, so little modifications were made in 3_slumbernet_full_training_Adapted.py in comparison to the original code.  
...
Configure desired filepaths and change parameters if needed/desired before building the Docker Image. Make sure the CMD command on the last line (line 24) in train_model.Dockerfile reads the following:
```
